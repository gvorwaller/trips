...
- **Viewer write exceptions**: viewers may toggle a packing item's `checked` state (`PATCH /api/packing/check`), a day-plan stop's `visited` state (`PATCH /api/dayplan/visited`), and a place's `visited` state (`PATCH /api/itinerary/visited`); each endpoint must verify nothing else changed. Everything else → 403.
